          ...
          <div className="grid lg:grid-cols-[240px_1fr] gap-12 lg:gap-16">
            ...
            <nav className="hidden lg:block" aria-label="Navigation expertises">
              <div className="sticky top-28 space-y-1">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-teal mb-4">
                  Domaines
                </p>
                  ...
                  <a
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${
                      activeSection === exp.id
                        ...
                        : "text-slate-400 hover:text-ocean hover:bg-ocean-50/50"
                    }`}
                  >
                    <span
                      className={`text-[10px] tabular-nums transition-colors duration-300 ${
                        activeSection === exp.id ? "text-teal" : "text-slate-300"
                      }`}
                    >
                    ...
                    {exp.shortTitle}
                    {activeSection === exp.id && (
                      <span className="ml-auto w-1.5 h-1.5 bg-teal rounded-full" />
                    )}
                  </a>
                ))}
              </div>
                ...
                >
                  <div className={`grid md:grid-cols-5 gap-10 ${index % 2 !== 0 ? "md:flex-row-reverse" : ""}`}>
                    ...
                    <div className={`md:col-span-3 ${index % 2 !== 0 ? "md:order-2" : ""}`}>
                      <div className="gsap-reveal">
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-14 h-14 bg-ocean-50 rounded-2xl flex items-center justify-center text-teal">
                          ...
                          <div>
                            ...
                            <h2 className="font-display text-fluid-3xl font-bold text-ocean leading-tight">
                              {exp.title}
                            </h2>
                          </div>
                        </div>
                      ...
                      <p className="gsap-reveal text-slate leading-relaxed text-fluid-base">
                        {exp.description}
                      </p>
                      ...
                      <div className="gsap-reveal mt-8">
                        ...
                        <ul className="space-y-3">
                          {exp.details.map((detail) => (
                            <li key={detail} className="flex items-start gap-3 text-sm text-slate">
                              <svg
                      ...
                      <div className="gsap-reveal mt-8">
                        <Link
                          href="/contact"
                          className="inline-flex items-center text-sm font-medium text-teal hover:text-teal transition-colors duration-300 group"
                        >
                    ...
                    <div className={`md:col-span-2 ${index % 2 !== 0 ? "md:order-1" : ""}`}>
                      <div className="gsap-reveal-scale sticky top-28">
                        <div className="bg-ocean-50/50 border border-ocean-100/40 rounded-3xl p-7">
                          <h3 className="font-display text-base font-bold text-ocean mb-5">
                          ...
                          <div className="space-y-3">
                            {exp.examples.map((example) => (
                              <div
                                key={example}
                                className="flex items-start gap-3 p-4 bg-white rounded-2xl border border-ocean-100/50 transition-all duration-300 hover:border-teal/20 hover:shadow-elevation-1"
                              >
                                <div className="w-8 h-8 bg-teal/8 rounded-lg flex items-center justify-center shrink-0">
                                  <svg
                                ...
                                <p className="text-sm text-ocean font-medium leading-snug">
                                  {example}
                                </p>
                              </div>
                            ))}
                          ...
                          <Link
                            href="/references"
                            className="mt-5 inline-flex items-center text-sm font-medium text-teal hover:text-teal transition-colors duration-300 group"
                          >
                  ...
                  {index < expertises.length - 1 && (
                    <div className="mt-24 lg:mt-32">
                      <div className="h-px bg-gradient-to-r from-transparent via-ocean-100 to-transparent" />
                    </div>
                  )}
                </div>
              ))}
