...
function StatCounter({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div className="text-center group">
      <span
        ref={ref}
        className="block font-display text-fluid-4xl font-bold text-teal transition-transform duration-300 group-hover:scale-105"
      >
        0{suffix}
      </span>
  ...
  return (
    <div ref={sectionRef}>
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        {/* Radial glow */}
        <div className="absolute top-1/4 right-1/4 w-[600px] h-[600px] bg-teal/5 rounded-full blur-[128px] animate-pulse-soft" />
        <div className="absolute bottom-1/4 left-1/6 w-[400px] h-[400px] bg-teal/5 rounded-full blur-[100px] animate-pulse-soft" style={{ animationDelay: "1.5s" }} />

        {/* Bottom gradient fade */}
        <div className="absolute bottom-0 left-0 w-full h-48 bg-gradient-to-t from-ocean-800/50 to-transparent" />
          ...
          <div className="max-w-4xl">
            ...
            <div className="gsap-reveal">
              <span className="inline-flex items-center gap-2 px-4 py-2 bg-white/[0.06] backdrop-blur-sm border border-white/[0.08] rounded-full text-white/70 text-fluid-xs font-medium tracking-wide">
                <span className="w-1.5 h-1.5 bg-teal rounded-full animate-pulse-soft" />
            ...
            <div className="gsap-reveal mt-10 flex flex-col sm:flex-row gap-4">
              <Link
                href="/contact"
                className="magnetic-btn inline-flex items-center justify-center px-8 py-4 bg-teal text-ocean font-semibold rounded-xl hover:bg-teal-400 transition-all duration-300 text-sm group"
              >
                Prendre rendez-vous
                <svg
      ...
      {/* ===== TRUST BAR / STATS ===== */}
      <section className="relative bg-ocean-700 py-16 lg:py-20 overflow-hidden">
        {/* Subtle top line */}
            ...
            {expertises.map((exp, i) => {
              ...
              return (
                <Link
                  ...
                  className={`${sizeClass} card-hover card-spotlight shimmer-border group relative p-8 bg-white border border-ocean-100/60 rounded-3xl overflow-hidden`}
                  data-stagger-child
                >
                  {/* Content */}
                  <div className="relative z-10">
                    <div className="w-12 h-12 bg-ocean-50 rounded-2xl flex items-center justify-center text-teal group-hover:bg-teal/10 group-hover:text-teal transition-all duration-300">
                      {getIcon(exp.icon, "w-6 h-6")}
                    </div>

                    <h3 className="mt-5 font-display text-fluid-xl font-bold text-ocean group-hover:text-ocean transition-colors">
                      {exp.shortTitle}
                    </h3>

                    <p className="mt-3 text-sm text-slate leading-relaxed line-clamp-3 lg:line-clamp-none">
                      {exp.description}
                    </p>
                    ...
                    {i === 0 && (
                      <ul className="mt-5 space-y-2 hidden lg:block">
                        {exp.details.slice(0, 3).map((detail) => (
                          <li key={detail} className="flex items-center gap-2 text-sm text-slate/70">
                            <span className="w-1 h-1 bg-teal rounded-full shrink-0" />
                            {detail}
                          </li>
                        ))}
                      </ul>
                    ...
                    <span className="mt-5 inline-flex items-center text-sm font-medium text-teal group-hover:text-teal transition-all duration-300">
                      En savoir plus
                      <svg
      ...
      {/* ===== WHY US ===== */}
      <section className="py-24 lg:py-32 bg-ocean-50/50 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            <div>
              ...
              <p className="gsap-reveal mt-6 text-slate leading-relaxed">
              ...
              <div className="gsap-reveal mt-8">
                <Link
                  href="/a-propos"
                  className="inline-flex items-center px-6 py-3 bg-ocean text-white font-semibold rounded-xl hover:bg-ocean-600 transition-all duration-300 text-sm group"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" data-stagger-parent>
              {whyUs.map((item) => (
                <div
                  key={item.title}
                  className="card-hover p-6 bg-white rounded-2xl border border-ocean-100/60"
                  data-stagger-child
                >
                  <div className="w-11 h-11 bg-teal/8 rounded-xl flex items-center justify-center text-teal">
                    <WhyUsIcon name={item.icon} />
                  </div>
                  <h3 className="mt-4 font-display text-lg font-bold text-ocean leading-tight">
                    {item.title}
                  </h3>
      ...
      <section className="py-24 lg:py-32 bg-white relative overflow-hidden">
        {/* Decorative */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-teal/3 rounded-full blur-[120px]" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 grid md:grid-cols-2 gap-8" data-stagger-parent>
            {testimonials.map((t) => (
              <div
                key={t.name}
                className="card-hover p-8 lg:p-10 bg-ocean-50/40 border border-ocean-100/40 rounded-3xl relative"
                data-stagger-child
              >
                {/* Quote mark */}
                <svg
                  className="absolute top-6 right-8 w-12 h-12 text-teal/10"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <blockquote className="relative z-10">
                  <p className="text-fluid-base text-ocean/80 leading-relaxed italic">
                    &ldquo;{t.quote}&rdquo;
                  </p>
      ...
      <section className="relative py-24 lg:py-32 overflow-hidden">
        ...
        {/* Gold glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-teal/5 rounded-full blur-[150px]" />

        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="gsap-reveal">
            <span className="inline-block text-teal/70 text-fluid-xs font-semibold uppercase tracking-[0.3em] mb-4">
              Passons a l&apos;action
            </span>
            <h2 className="font-display text-fluid-4xl font-bold text-white leading-tight text-balance">
              Un projet ? Une question ?
              <br />
              <span className="text-teal">Parlons-en.</span>
            </h2>
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/contact"
                className="magnetic-btn inline-flex items-center justify-center px-8 py-4 bg-teal text-ocean font-semibold rounded-xl hover:bg-teal-400 transition-all duration-300 text-sm"
              >
                Prendre rendez-vous
              </Link>
              <Link
