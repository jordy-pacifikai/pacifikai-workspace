    ...
    >
      ...
      <div className="relative h-64 sm:h-72 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4">
          <h3 className="font-heading text-2xl text-white font-bold">{room.name}</h3>
        </div>
        {room.featured && (
          <div className="absolute top-4 right-4 bg-ocean-500 text-white text-xs font-bold px-3 py-1 rounded-full">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div>
            ...
            <p className="text-ocean-500 font-heading text-xl font-bold">{room.priceLabel}</p>
          </div>
          <Link
            href={`/reservation?room=${room.id}`}
            className="bg-ocean-500 text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-ocean-600 transition-colors"
          >
            {t.rooms.book}
          </Link>
        </div>
