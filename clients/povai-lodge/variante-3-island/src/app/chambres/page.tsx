    ...
    <>
      ...
      <div ref={sectionRef}>
        ...
        {rooms.map((room: Room, i: number) => (
          <section
            key={room.id}
            className={`py-20 ${i % 2 === 1 ? "bg-cream-50" : "bg-white"}`}
          >
              ...
              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center ${i % 2 === 1 ? "lg:grid-flow-dense" : ""}`}>
                ...
                <div className="animate-on-scroll">
                  <div className="flex items-center gap-3 mb-2">
                    {room.featured && (
                      <span className="bg-sunset-100 text-sunset-700 text-xs font-bold px-3 py-1 rounded-full">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                    <div className="bg-cream-100 rounded-xl p-4 text-center">
                      <svg className="w-6 h-6 text-sunset-500 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* Price & CTA */}
                  <div className="flex items-center justify-between p-6 bg-sunset-50 rounded-2xl">
                    <div>
                      <span className="text-sm text-gray-500">{t.rooms.from}</span>
                      <p className="font-serif text-2xl sm:text-3xl font-bold text-sunset-600">
                        {room.priceLabel}
                      </p>
                    </div>
                    <Link
                      href={`/reservation?room=${room.id}`}
                      className="bg-sunset-500 text-white px-6 py-3 rounded-full font-semibold hover:bg-sunset-600 transition-all duration-300 hover:scale-105"
                    >
                      {t.rooms.book}
                    </Link>
                  </div>
