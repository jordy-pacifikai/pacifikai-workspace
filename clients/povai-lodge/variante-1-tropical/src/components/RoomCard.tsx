  ...
  return (
    ...
    >
      ...
      <div className="relative h-64 sm:h-72 overflow-hidden">
        ...
        {room.featured && (
          <div className="absolute top-4 right-4 bg-turquoise-500 text-white text-xs font-bold px-3 py-1 rounded-full">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
          <span className="flex items-center gap-1">
            <svg className="w-4 h-4 text-turquoise-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div>
            ...
            <p className="text-turquoise-500 font-serif text-xl font-bold">{room.priceLabel}</p>
          </div>
          <Link
            href={`/reservation?room=${room.id}`}
            className="bg-turquoise-500 text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-turquoise-600 transition-colors"
          >
            {t.rooms.book}
          </Link>
        </div>
