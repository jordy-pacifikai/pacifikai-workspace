...
var BOOKBOT_TOOLS = [
  {
    ...
    function: {
      ...
      description: "Cherche dans la base de connaissances du commerce (FAQ, politiques, détails services, informations pratiques). Utilise cet outil pour répondre aux questions spécifiques du client.",
      parameters: {
        type: "object",
        properties: {
          query: {
  ...
  },
  {
    ...
    function: {
      name: "list_services",
      description: "Liste tous les services disponibles avec leurs prix et durées. Appelle cet outil quand le client demande ce qui est proposé.",
      parameters: {
        type: "object",
        properties: {}
      }
  ...
  },
  {
    ...
    function: {
      ...
      description: "Vérifie les créneaux disponibles pour un service à une date donnée. Prend en compte les RDV existants pour ne proposer que les créneaux réellement libres.",
      parameters: {
        type: "object",
        properties: {
          service_name: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Confirme et crée un rendez-vous. IMPORTANT : tu DOIS avoir demandé confirmation au client AVANT d'appeler cet outil.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Annule le prochain rendez-vous confirmé du client. Demande confirmation avant d'annuler.",
      parameters: {
        type: "object",
        properties: {}
      }
  ...
  },
  {
    ...
    function: {
      ...
      description: "Transfère la conversation à un humain quand la demande dépasse tes capacités ou que le client le demande explicitement.",
      parameters: {
        type: "object",
        properties: {
          reason: {
...
__name(getTahitiDate, "getTahitiDate");
function getTahitiDateISO() {
...
__name(getTahitiDateISO, "getTahitiDateISO");
function buildSystemPrompt(config, today) {
  const services = parseAllServices(config.services).map(
    (s) => `- ${s.name} (${s.duration} min, ${s.price.toLocaleString("fr-FR")} XPF)`
  ).join("\n");
  const dayLabels = {
    mon: "Lundi",
    tue: "Mardi",
  ...
  const hours = Object.entries(config.openingHours).map(([day, h]) => `${dayLabels[day] ?? day}: ${h}`).join("\n");
  const cfg = config.chatbotConfig ?? {};
  const tone = cfg.tone === "formel" ? "formel et professionnel" : cfg.tone === "decontracte" ? "décontracté et amical" : 'chaleureux et accueillant, utilise "Ia ora na" pour saluer';
  const langMap = {
    fr: "français",
    en: "anglais",
    tah: "tahitien (reo Māʼohi)"
  };
  const language = langMap[cfg.language ?? "fr"] ?? "français";
...
- Quand un client te salue, utilise cette formule de bienvenue : "${greeting}"` : "";
  const customInstructions = cfg.custom_instructions ?? "";
...
async function executeTool(name, input, clientPhone, config) {
  switch (name) {
    case "search_knowledge_base": {
      ...
      return results.map((r) => `[${r.title}] ${r.chunk_text}`).join("\n\n");
    }
    case "list_services": {
    ...
    case "check_availability": {
      ...
      if (config.gcalRefreshToken && config.gcalCalendarId) {
        try {
          const requestedDate = /* @__PURE__ */ new Date(date + "T00:00:00");
          const twoDaysFromNow = /* @__PURE__ */ new Date();
...
async function runBookingAgent(session, payload, config) {
  ...
  return finalReply || "Désolé, je n'ai pas pu traiter ta demande. Réessaie ou écris 'humain' pour parler à quelqu'un.";
}
__name(runBookingAgent, "runBookingAgent");

export {
  runBookingAgent
};
//# sourceMappingURL=chunk-PI6QB4QI.mjs.map
