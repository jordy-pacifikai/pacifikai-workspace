...
function AppointmentRow({ appt, isToday }: { appt: Appointment; isToday: boolean }) {
  const displayName =
    appt.client?.name ?? appt.guest_name ?? 'Client inconnu';

  const serviceName = appt.service?.name ?? '—';

  const timeLabel = appt.start_time.slice(0, 5); // "HH:MM"
...
export default function DashboardPage() {
  ...
  // Sort today ascending by start_time, then tomorrow ascending
